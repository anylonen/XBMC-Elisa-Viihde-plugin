    ...
    if len(paramstring) >= 2:
        ...
        for i in range(len(pairsofparams)):
            ...
            if (len(splitparams)) == 2:
                param[splitparams[0]] = splitparams[1].replace('_ampresand_', '&').replace(
...
def show_dir(dirid=0):
    ...
    for row in data:
        ...
        add_watch_link(name,
                       ...
                       totalItems,
                       kwargs={
                           "title": name.replace('"', '\'\'').replace('&', '_ampresand_').replace('<', '_lessthan_').replace('>', '_greaterthan_'),
                           ...
                           "plotoutline": (row['description'] if "description" in row else "N/a").encode('utf8').replace('"', '\'\'').replace('&', '_ampresand_').replace('<',
                                                                                                                                                                          '_lessthan_').replace('>', '_greaterthan_'),
                           "plot": str((row['serviceName'] if "serviceName" in row else "N/a").encode('utf8') + '\n' +
                                       ...
                                       (row['description'] if "description" in row else "N/a").encode('utf8')).replace('"', '\'\'').replace('&', '_ampresand_').replace('<', '_lessthan_').replace('>', '_greaterthan_'),
